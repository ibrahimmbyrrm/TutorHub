...
struct FirebaseDataManager : FirebaseService {
    func getData(completion : @escaping([Announcement]?)->Void) {
        ...
        firestoreDatabase.collection("Announcements").order(by: "date", descending: true).addSnapshotListener { (snapshot, error) in
            ...
            for document in snapshot!.documents {
                ...
                let announcement = Announcement(title: title, name: name, location: location, type: type, address: address, description: description, phone: phone, cost: cost, announcer: announcer)
